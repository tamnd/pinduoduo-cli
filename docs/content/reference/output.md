...
describes what users get. Pick a format with `-o`, or let pdd choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pdd <command> -o table   # aligned columns for reading
pdd <command> -o jsonl   # one JSON object per line, for piping
pdd <command> -o json    # a single JSON array
pdd <command> -o csv     # spreadsheet friendly
pdd <command> -o tsv     # tab-separated
pdd <command> -o url     # just the URL column
pdd <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pdd <command> --fields id,title,url
```
...
```bash
pdd <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
pdd <command>            # a table, because this is a terminal
pdd <command> | wc -l    # JSONL, because this is a pipe
```
